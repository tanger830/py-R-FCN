...
def parse_rec(imagename):
    filename = imagename.replace('images', 'labels').replace('png','txt').replace('jpg', 'txt')
    ...
    for ix, obj in enumerate(objs):
        ...
        cls = int(row[0])
        obj_struct = {}
        #obj_struct['name'] = str(int(row[0]))
        obj_struct['name'] = 'vehicle'
...
def vehicle_eval(detpath,
    ...
    if not os.path.isdir(cachedir):
        os.mkdir(cachedir)
    cachefile = os.path.join(cachedir, 'vehicle_annots.pkl')
    # read list of images
